const CONFIG = {
  // index.html と manifest.json はこのWebViewerフォルダ内に置く。
  manifestPath: './manifest.json',

  // 現在の配置:
  // repo/
  //   999.Cthulhu/
  //     md-webviewer-starter-nested-fixed/
  //       index.html
  //       manifest.json
  //
  // manifest.json の path は "999.Cthulhu/..." なので、
  // index.html から2つ上の repo/ へ戻して読み込む。
  contentBasePath: '../../'
};
...
function buildContentUrl(path) {
  const base = CONFIG.contentBasePath.endsWith('/')
    ? CONFIG.contentBasePath
    : `${CONFIG.contentBasePath}/`;
  return `${base}${normalizeManifestPath(path)}`;
}
...
function renderToc() {
  ...
  state.manifest.forEach(group => {
    ...
    group.files.forEach(file => {
      ...
      const globalIndex = state.flatFiles.findIndex(item => item.path === normalizedPath);
...
async function loadMarkdown(index) {
  ...
  const mdUrl = buildContentUrl(item.path);
  const res = await fetch(encodeURI(mdUrl), { cache: 'no-store' });
  if (!res.ok) {
    throw new Error(`${mdUrl} を読み込めません: ${res.status}\nmanifest path: ${item.path}`);
  }
...
function showError(err) {
  ...
  els.content.innerHTML = `
    <h1>読み込みエラー</h1>
    <p>${escapeHtml(err.message).replaceAll('\n', '<br>')}</p>
    <p>この版は、<code>md-webviewer-starter</code> フォルダ内の <code>index.html</code> から、1つ上の階層にある <code>999.Cthulhu</code> を読み込む設定です。</p>
    <pre>repo/
├─ 999.Cthulhu/
